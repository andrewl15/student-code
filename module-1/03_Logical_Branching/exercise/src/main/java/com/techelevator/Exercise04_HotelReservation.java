...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        return numberOfNights >= 1 && numberOfNights < 3 ? DAILY_RATE * numberOfNights : DISCOUNT_RATE * numberOfNights;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        double parkingAndNightCalc = numOfTotalNights;
        if(includesParking){
            parkingAndNightCalc *= PARKING_RATE;
        }
        return includesParking ? calculateStayTotal(numOfTotalNights) + parkingAndNightCalc : calculateStayTotal(numOfTotalNights);
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking, boolean includesLateCheckout) {
        return includesLateCheckout ? calculateStayTotal(numOfTotalNights, includesParking) + LATE_CHECKOUT_FEE : calculateStayTotal(numOfTotalNights, includesParking);
    }
}
