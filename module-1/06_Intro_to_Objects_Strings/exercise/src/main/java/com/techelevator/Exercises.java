...
public class Exercises {
	...
	public boolean doubleX(String str) {
		...
		if(str.contains("x")) {
			if (str.indexOf("x") != str.length() - 1) {
				isDoubleX = str.charAt(str.indexOf("x") + 1) == 'x';
			}
		}
		return isDoubleX;
	}
	...
	public String stringBits(String str) {
		...
		if(str.length() % 2 != 0){
			everyOtherChar += str.charAt(str.length() - 1);
		}
	...
	public String stringX(String str) {
		...
		return  str.length() < 2 ? str :
				str.charAt(0) == 'x' && str.charAt(str.length() - 1) == 'x' ? "x" + str.substring(1, str.length() - 1).replace("x", "") + "x" :
				str.contains("x") ? str.replace("x", "") : str;
	}
