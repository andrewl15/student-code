...
public class Exercises {
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		list.removeIf(listItem -> listItem.length() == 4);
		return list;
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		list.removeIf(listItem -> listItem % 2 == 0);
		return list;
	}
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		int shortestList = Math.min(listOne.size(), listTwo.size());
		List<Integer> list = new ArrayList<Integer>();
		for(int i = 0; i < shortestList; i++){
			list.add(listOne.get(i));
			list.add(listTwo.get(i));
		}
		if (listOne.size() > shortestList) {
			list.addAll(listOne.subList(shortestList, listOne.size()));
		} else if (listTwo.size() > shortestList) {
			list.addAll(listTwo.subList(shortestList, listTwo.size()));
		}
		return list;
	}
