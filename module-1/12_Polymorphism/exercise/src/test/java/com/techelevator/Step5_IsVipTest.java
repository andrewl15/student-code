...
public class Step5_IsVipTest {
    ...
    private String classWellFormedCheck() {
        ...
        String constructorCheck = ReflectionTestHelper.checkDefaultConstructor(bankCustomerClass);
        if (!constructorCheck.isEmpty()) {
            return constructorCheck;
        }

        // Assert accounts field exists with correct type and access
        String fieldCheck = ReflectionTestHelper.checkField(bankCustomerClass, "accounts", java.util.List.class, ReflectionTestHelper.AccessModifier.Private);
        if (!fieldCheck.isEmpty()) {
            return fieldCheck;
        }

        // Assert getAccounts method exists
        String methodCheck = ReflectionTestHelper.checkMethod(bankCustomerClass, "getAccounts", Array.newInstance(accountableInterfaceClass, 0).getClass(), true, new Class[] { });
    ...
    private String methodWellFormedCheck() {
        ...
        // Assert class exists and is well-formed
        String wellFormedCheck = classWellFormedCheck();
        if (!wellFormedCheck.isEmpty()) {
            return wellFormedCheck;
        }
